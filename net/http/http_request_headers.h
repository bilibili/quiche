...
/* 
   1. remove deps on net/base/net_export.h
   2. remove unsed net/log/net_log_capture_mode.h
   3. switch StringPiece to Quiche::QuicheStringPiece
*/
...
#ifndef QUICHE_NET_HTTP_HTTP_REQUEST_HEADERS_H_
#define QUICHE_NET_HTTP_HTTP_REQUEST_HEADERS_H_
...
class HttpRequestHeaders {
 public:
  struct HeaderKeyValuePair {
    HeaderKeyValuePair();
    HeaderKeyValuePair(const gurl_base::StringPiece& key,
  ...
  bool HasHeader(const gurl_base::StringPiece& key) const {
  ...
  bool GetHeader(const gurl_base::StringPiece& key, std::string* out) const;
  ...
  void SetHeader(const gurl_base::StringPiece& key, const gurl_base::StringPiece& value);

  // Does the same as above but without internal DCHECKs for validations.
  void SetHeaderWithoutCheckForTesting(const gurl_base::StringPiece& key,
                                       const gurl_base::StringPiece& value) {
  ...
  // The caller must ensure that |key| passes HttpUtil::IsValidHeaderName() and
  // |value| passes HttpUtil::IsValidHeaderValue().
  void SetHeaderIfMissing(const gurl_base::StringPiece& key,
                          const gurl_base::StringPiece& value);

  // Removes the first header that matches (case insensitive) |key|.
  void RemoveHeader(const gurl_base::StringPiece& key);
  ...
  // field-content.
  void AddHeaderFromString(const gurl_base::StringPiece& header_line);
  ...
  void AddHeadersFromString(const gurl_base::StringPiece& headers);
 ...
 private:
  HeaderVector::iterator FindHeader(const gurl_base::StringPiece& key);
  HeaderVector::const_iterator FindHeader(const gurl_base::StringPiece& key) const;

  void SetHeaderInternal(const gurl_base::StringPiece& key,
                         const gurl_base::StringPiece& value);
...
#endif  // QUICHE_NET_HTTP_HTTP_REQUEST_HEADERS_H_
