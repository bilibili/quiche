...
#ifndef QUICHE_QUIC_CORE_BATCH_WRITER_QUIC_XSK_BATCH_WRITER_BASE_H_
#define QUICHE_QUIC_CORE_BATCH_WRITER_QUIC_XSK_BATCH_WRITER_BASE_H_
...
class QuicXskBatchWriterBase : public QuicPacketWriter {
 public:
  explicit QuicXskBatchWriterBase(
  ...
  QuicPacketBuffer GetNextWriteLocation(
    ...
    char* loc = batch_buffer_->GetNextWriteLocation(self_address, nullptr);
    if (loc == nullptr) {
      complete_tx(xsk_);
    }
    return {loc, nullptr};
 ...
 protected:
  const QuicXskBatchWriterBuffer& batch_buffer() const { return *batch_buffer_; }
  QuicXskBatchWriterBuffer& batch_buffer() { return *batch_buffer_; }
  ...
  xsk_socket_info* xsk() { return xsk_; }
 ...
 private:
  ...
  std::unique_ptr<QuicXskBatchWriterBuffer> batch_buffer_;
};

// QuicXskUdpBatchWriter is a batch writer backed by a xsk UDP socket.
class QuicXskUdpBatchWriter : public QuicXskBatchWriterBase {
 public:
  QuicXskUdpBatchWriter(std::unique_ptr<QuicXskBatchWriterBuffer> batch_buffer,
                    xsk_socket_info* xsk, int fd)
      : QuicXskBatchWriterBase(std::move(batch_buffer), xsk), fd_(fd) {}
...
#endif  // QUICHE_QUIC_CORE_BATCH_WRITER_QUIC_XSK_BATCH_WRITER_BASE_H_
