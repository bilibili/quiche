...
#include "gquiche/quic/core/quic_legacy_version_encapsulator.h"
...
#include "gquiche/common/platform/api/quiche_flag_utils.h"
...
bool QuicConnection::OnAckFrameStart(QuicPacketNumber largest_acked,
  ...
  if (debug_visitor_ != nullptr) {
    debug_visitor_->OnAckFrameStart(ack_delay_time);
  }  
  return true;
}
...
bool QuicConnection::OnAckRange(QuicPacketNumber start, QuicPacketNumber end) {
  ...
  if (debug_visitor_ != nullptr) {
    debug_visitor_->OnAckRange(start, end);
  }  
  return true;
}
...
bool QuicConnection::OnAckFrameEnd(QuicPacketNumber start) {
  ...
  if (debug_visitor_ != nullptr) {
    debug_visitor_->OnAckFrameEnd(start);
  }
...
void QuicConnection::OnPacketComplete() {
  if (debug_visitor_ != nullptr) {
    debug_visitor_->OnPacketComplete();
  }	
