...
class ConnectionDebugVisitor : public quic::QuicConnectionDebugVisitor {
 public:
  ConnectionDebugVisitor(quic::FileQLogger* qlogger,
                              quic::QuicConnection* connection);
  ~ConnectionDebugVisitor() override {}
  ...
  // Called for QUIC+TLS versions_ when we send transport parameters.
  virtual void OnTransportParametersSent(
      const quic::TransportParameters& /*transport_parameters*/) override {}

  // Called for QUIC+TLS versions_ when we receive transport parameters.
  virtual void OnTransportParametersReceived(
      const quic::TransportParameters& /*transport_parameters*/) override {}

  // Called for QUIC+TLS versions_ when we resume cached transport parameters for
  // 0-RTT.
  virtual void OnTransportParametersResumed(
      const quic::TransportParameters& /*transport_parameters*/) override {}

  // Called for QUIC+TLS versions_ when 0-RTT is rejected.
  virtual void OnZeroRttRejected(int /*reject_reason*/) override {}

  // Called for QUIC+TLS versions_ when 0-RTT packet gets acked.
