...
QuicLogEmitter::QuicLogEmitter(QuicLogLevel level, std::string file, int line)
  : level_(level), file_(file), line_(line), saved_errno_(errno) {}

QuicLogEmitter::~QuicLogEmitter() {
  if (is_perror_) {
    // TODO(wub): Change to a thread-safe version of errorDetails.
    stream_ << ": " << saved_errno_ << " [" << saved_errno_ << "]";
  }
  ...
  GetLogger().log(spdlog::source_loc{file_.c_str(), line_, SPDLOG_FUNCTION}, level_, content.c_str());
